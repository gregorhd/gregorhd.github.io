---
layout: post
title: Remote sensing urban growth in Nakuru, Kenya (2) | geemap + Google Earth Engine
...
- [Imports and region of interest](#imports-and-region-of-interest)
...
- [Simplify land cover classes](#simplify-land-cover-classes)
...
## Imports and region of interest
...
print("Copernicus date of issuance: ", ee.Date(copernicus_lc.get('system:time_start')).format('YYYY-MM-dd').getInfo())

```

    Landsat 8 date of acquisition:  2018-10-03
    Copernicus date of issuance:  2019-01-01
...
# Here we chose the Random Forest algorithm with 15 decision trees.
...
This is where the rubber hits the road and we actually classify a Landsat 8 scene based on the Copernicus training data we just collected. The scene we're going to classify, however, will not be the one we used for training but instead the  exact same scene we manually classified in Part 1.
...
But first, we need to understand which of the numeric values correspond to which real-world land cover classes. For this we extract the class names and values form the original Copernicus dataset, and 'zip' these two lists into one.
...
Lastly, we define legend keys and define an ordered color palette which will inform both our `vis_params` in the `addLayer` method as well as the symbology for our legend.
...
Everything from here on out is going to be a replication from above, so no surprises there.
